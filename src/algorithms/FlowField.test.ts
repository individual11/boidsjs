...
describe('FlowField', () => {
    it('should calculate angle from noise and apply force to boid', () => {
        ...
        // angle = 0.125 * Math.PI * 4 = 0.5 * Math.PI (90 degrees)
    ...
    it('should multiply z by timeShift when sampling noise', () => {
        const boid = new Boid({
        ...
        const noise = new Noise();
        const noiseSpy = vi.spyOn(noise, 'noise').mockReturnValue(0);
        boid.applyForce = vi.fn();

        const flowField = new FlowField();
        const options: FlowFieldOptions = { scale: 0.1, strength: 0.05, timeShift: 3 };

        flowField.apply(boid, noise, options, 2);

        // z * timeShift = 2 * 3 = 6
        expect(noiseSpy).toHaveBeenCalledWith(0, 0, 6);
    });

    it('should use default timeShift of 1 when not specified', () => {
        const boid = new Boid({
            position: new Vector(0, 0),
            velocity: new Vector(0, 0),
            acceleration: new Vector(0, 0),
        ...
        flowField.apply(boid, noise, options, 5);

        // z * 1 = 5 (unchanged)
        expect(noiseSpy).toHaveBeenCalledWith(0, 0, 5);
    });
});
